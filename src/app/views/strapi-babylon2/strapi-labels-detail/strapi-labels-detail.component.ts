...
export class StrapiLabelsDetailComponent implements OnInit, OnDestroy {
  ...
  private controlPathIds(mode: number, id: number): void {
    this.strapiService
      .getTableCollectionItem('application::label.label', id)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(
        (responseData: any) => {
          this.dataRes = responseData;
          this.inputsItemsMerge = Object.entries(this.dataRes)
            .map((el: any) => el)
            .filter((elF: any) => elF[0].includes('_body'))
            .map((elM: any) => ({ key: elM[0], value: elM[1] }));
          this.selectsItemsMerge = Object.entries(this.dataRes)
            .map((el: any) => el)
            .filter((elF: any) => Array.isArray(elF[1]))
            .map((elM: any) => ({ key: elM[0], value: elM[1] }));
        },
        (error: any) => {
          console.log(error);
  ...
  private deeplTranslateAction(): void {
    this.deeplService
      .translateDeepL(/* this.key */ '', 'IT', 'EN-GB')
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(
        (responseData: any) => {
  ...
  public getItem(element: any, origin: string = 'it'): any {
    return {
      el: element,
      can: element.key?.split('_')[0].trim()?.length > 0,
      origin: element.key?.split('_')[0] === origin,
      length: element.value != null ? element.value.trim()?.length : 0,
    };
  }
  ...
  public translateAllLabels(): void {
    const labels = [
      { key: 'label_title', value: this.dataRes['label_title'] },
      ...this.inputsItemsMerge,
    ];

    if (!labels.every((res: any) => res.value != null)) {
      const _keys = labels.filter((el: any, index: number) => el.value == null);
      const _queryKeys = _keys?.map((elQuery: any) =>
        this.translateAction({ key: elQuery.key, value: elQuery.value })
      );
  ...
  public deleteLabelKey(): void {
    if (confirm('Are you sure you want to delete this entry?')) {
      this.strapiService
        .deleteTableCollectionItem('application::label.label', this.pId)
        .pipe(takeUntil(this.unsubscribe$))
  ...
  public clearFields(): void {
    this.inputsItemsMerge = Object.entries(this.dataRes)
      ...
      .map((elM: any) => ({
        key: elM[0],
        value: elM[0] === 'it_body' ? elM[1] : null,
      }));
  }
    ...
    if (confirm(`Confirm save labels in ${this.dataRes.label_title}?`)) {
      const data = {
        ...
        ...Object.assign(
          {},
          this.dataRes,
          ...this.inputsItemsMerge.reduce((acc: any, item: any, index: any) => {
            if (
              this.dataRes[item.key] ===
              this.inputsItemsMerge.find((f: any) => f.key == item.key)?.value
            ) {
              ...
              acc = [
                ...acc,
                {
                  [item.key]: this.inputsItemsMerge.find(
                    (f: any) => f.key == item.key
                  )?.value,
                },
              ];
            }
      ...
      if (this.pId != this.qId) {
        this.strapiService
          .updateTableCollectionItem('application::label.label', data, this.pId)
          .pipe(takeUntil(this.unsubscribe$))
          .subscribe((responseData: any) => {
        ...
        this.strapiService
          .cloneTableCollectionItem('application::label.label', data)
          .pipe(takeUntil(this.unsubscribe$))
          .subscribe((responseData: any) => {
            this.router.navigate(['strapi', 'labels']);
          }),
